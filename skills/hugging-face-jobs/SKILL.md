...
- **HF_TOKEN for Hub Access** ?? CRITICAL - Required for any Hub operations (push models/datasets, download private repos, etc.)
...
**?? CRITICAL:** The `$HF_TOKEN` placeholder is automatically replaced with your logged-in token. Never hardcode tokens in scripts.
...
hf_jobs("uv", {
    "script": "your_script.py",
    "secrets": {"HF_TOKEN": "hf_abc123..."}  # ?? Hardcoded token
})
```
...
hf_jobs("uv", {
    "script": "your_script.py",
    "env": {"HF_TOKEN": "hf_abc123..."}  # ?? Less secure than secrets
})
```
...
?? **Important:** There are *two* "script path" stories depending on how you run Jobs:
...
**?? EPHEMERAL ENVIRONMENT—MUST PERSIST RESULTS**
