...
| 依赖 | 用途 | 安装命令 |
|------|------|----------|
| `curl` | HTTP请求 | 系统自带或包管理器安装 |
| `jq` | JSON解析和处理 | `apt/brew install jq` |
| `HF_TOKEN` | API认证（提高速率限制） | 从Hugging Face设置获取 |

### 可选依赖（按脚本类型）
| 脚本类型 | 额外依赖 | 说明 |
|----------|----------|------|
| Bash脚本 | `hf` CLI | `uv tool install huggingface-hub` |
| Python脚本 | `huggingface-hub`, `requests` | `pip install huggingface-hub requests` |
| TypeScript脚本 | `tsx`, `@huggingface/hub` | `npm install -g tsx` |
...
└─────────────────────────────────────────────────────────┘
        ...
        ▼                   ▼                   ▼
┌───────────────┐   ┌───────────────┐   ┌───────────────┐
│   Bash脚本     │   │  Python脚本   │   │ TypeScript脚本│
│   (.sh)       │   │   (.py)       │   │   (.tsx)      │
├───────────────┤   ├───────────────┤   ├───────────────┤
│ ? curl        │   │ ? requests    │   │ ? tsx         │
│ ? jq          │   │ ? huggingface │   │ ? @huggingface│
│ ? hf CLI      │   │   -hub        │   │   /hub        │
└───────────────┘   └───────────────┘   └───────────────┘
        │                   │                   │
        └───────────────────┼───────────────────┘
...
# 4. 设置HF_TOKEN
export HF_TOKEN="hf_xxx"  # 从 https://huggingface.co/settings/tokens 获取
```
