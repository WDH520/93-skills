...
- **HF_TOKEN for Hub Push** ?? CRITICAL - Training environment is ephemeral, must push to Hub or ALL training results are lost
...
**?? IMPORTANT: Training jobs run asynchronously and can take hours**
...
?? **Important:** The `script` parameter accepts either inline code (as shown above) OR a URL. **Local file paths do NOT work.**
...
?? **In Claude Code context, prefer using `hf_jobs()` MCP tool (Approach 1) when available.**
